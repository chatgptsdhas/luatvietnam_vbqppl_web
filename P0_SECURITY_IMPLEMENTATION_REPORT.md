...
| `apps_script/Security.js` | **Gỡ bỏ hoàn toàn** fallback `legacy_token_fallback` trong `validateServiceToken_` — chỉ còn chấp nhận đúng `APPS_SCRIPT_SERVICE_TOKEN` (Script Property bắt buộc, không optional nữa). |
...
- `tests/test_p0_static_checks.py` — 30 test tĩnh (không còn default token/password/fallback, redact, admin session bắt buộc, script Python gửi service_token...).
...
- `.github/workflows/security.yml` — compile check + syntax check Apps Script + unit test Python + test hành vi Node + chặn file cấm + quét secret khi push/PR.
...
## 6. Test đã chạy (lần chạy cuối cùng, sau khi gỡ fallback service token)
...
- `python -m unittest discover -s tests -p "test_*.py"` → **57/57 PASS** (`Ran 57 tests ... OK`):
  ...
  - 30 test tĩnh cho `WebApp.js`, `Security.js`, `Dashboard/index.html`, `planner_sync_server.py`,
    và 3 script Python gửi `service_token` (`test_p0_static_checks.py`, gồm test mới
    `test_no_service_token_fallback_to_legacy_token` +
...
- `.github/workflows/security.yml` đã được xác thực cú pháp YAML hợp lệ (parse bằng PyYAML,
  10 step — bổ sung Setup Node.js + syntax check Apps Script + chạy
  `test_webapp_security_boundaries.js` — trigger đúng `push`/`pull_request`) — **chưa** được chạy
  thật trên GitHub (không push).
...
- [ ] Đã chạy `python scripts/generate_p0_secrets.py` và điền đủ Script Properties +
      `.env` theo `P0_MANUAL_ACTIONS.md` trên môi trường **test** trước, xác nhận đăng nhập
      admin + transfer/update + Planner sync hoạt động đúng.
