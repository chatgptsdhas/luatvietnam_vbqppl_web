...
|---|---|---|
| Lưu phiên đăng nhập | `01_save_session.py` | Mở trình duyệt thật để đăng nhập luatvietnam.vn, lưu `auth/luatvietnam_state.json` cho các bước sau |
| Thu thập danh sách văn bản | `07_crawl_luatvietnam_list_by_field.py` | Crawl danh sách văn bản theo `ma_linh_vuc` trong `config/scan_config.json`, ghi `output/field_document_urls.json` |
...
1. `01_save_session.py` nếu nghi ngờ phiên đăng nhập `auth/luatvietnam_state.json` hết hạn/không hợp lệ
2. `07_crawl_luatvietnam_list_by_field.py` và `output/field_document_urls.json` nếu danh sách URL đầu vào thiếu/sai
3. `08_process_field_documents_batch.py`
4. `config/scan_config.json`
5. `apps_script/WebApp.js`
6. `apps_script/Security.js`
7. log có correlation ID hoặc output runtime được người dùng cung cấp
